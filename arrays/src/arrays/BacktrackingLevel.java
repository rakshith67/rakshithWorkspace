...
public class BacktrackingLevel {
	...
	public List<List<Integer>> combinationSum3(int k, int n) {
		List<List<Integer>> list = new ArrayList<>();
		fillListWithCombSum(list, new ArrayList<>(), k, n, 1);
		return list;
	}

	private void fillListWithCombSum(List<List<Integer>> result, List<Integer> list, int k, int n, int start) {
		if (n == 0 && k == 0) {
			result.add(new ArrayList<>(list));
			return;
		}
		...
		for (int i = start; i <= 9; i++) {
			list.add(i);
			fillListWithCombSum(result, list, k - 1, n - i, i + 1);
			list.remove(list.size() - 1);
		}
	}
